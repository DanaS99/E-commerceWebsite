...
router.get('/:email', async (req, res) => {
  ...
  if (!email) {
    return res.status(400).send.json({ message: 'Email is required' });
  }

  try {
    const orders = await Order.find({ email: email });
    if (orders.length === 0 || !orders) {
      return res.status(400).send.json({ order: 0, message: 'Orders not found for this email' });
    } 
...
router.get('/order/:id', async (req, res) => {
  ...
  try {
    ...
    if (!order) {
      return res.status(404).send.json({ message: 'Order not found' });
    }
...
router.get('/',async (req, res) => {
  try {
    ...
    if (orders.length === 0) {
      return res.status(400).send.json({ orders: [], message: 'No orders found' });
    }
    res.status(200).send(orders)
  } catch (error) {
...
router.patch('/update-order-status/:id', async (req, res) => {
  ...
  if (!status) {
    return res.status(400).send.json({ message: 'Status is required' });
  }
